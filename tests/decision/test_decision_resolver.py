...
def test_resolver_combines_context_and_ontology_into_a_recommendation(resolver):
    decision = resolver.resolve(
        observation("scalpel"),
        context(lifecycle="reusable", contamination_state="not-regulated"),
    )
...
def test_resolver_reports_information_required_by_policy(resolver):
    decision = resolver.resolve(
        observation("scalpel"),
        context(contamination_state="not-regulated"),
    )
...
def test_reusable_nonsharp_item_is_sent_to_reprocessing(resolver):
    decision = resolver.resolve(
        observation("tray"),
        context(contamination_state="not-regulated"),
    )
    ...
    assert decision.action == "transport-to-reprocessing"
    assert decision.matched_rule == "reusable-nonsharp"


def test_resolver_reports_uncovered_known_facts(resolver):
    decision = resolver.resolve(
        observation("tray"),
        context(contamination_state="chemical"),
    )
...
def test_recommendation_carries_all_artifact_versions(resolver):
    decision = resolver.resolve(
        observation("scalpel"),
        context(lifecycle="single-use", contamination_state="not-regulated"),
    )
